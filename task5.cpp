...
{
    Mat input_img = imread("C:/Users/andre/OneDrive/Desktop/Computer_Vision/Lab_3/robocup.jpg");
...
void onMouse(int event, int x, int y, int f, void *userdata)
{
    ...
    if(event == EVENT_LBUTTONDOWN)
    {
        ...
        for(int i = 0; i < img_out.rows; i++)
        {
            for(int j = 0; j < img_out.cols; j++)
            {
                ...
                if(dist_fromB <= treshold && dist_fromG <= treshold && dist_fromR <= treshold)
                {
                    ...
                    mask2.at<Vec3b>(i, j) = Vec3b (blueVal, greenVal, redVal);
                }else{
                    mask.at<uchar>(i, j) = 0;
                }
        ...
        imshow("Mask", mask);
        imshow("Mask with setted color", mask2);
    }
}